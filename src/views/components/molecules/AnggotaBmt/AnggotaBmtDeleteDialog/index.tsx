...
type AnggotaBmtDeleteDialogProps = {
  row: AnggotaBmt | null;
  isDeleting: boolean;
  onCancel: () => void;
  onConfirm: () => void;
...
export default function AnggotaBmtDeleteDialog({
  isDeleting,
  onCancel,
  onConfirm,
  row,
}: AnggotaBmtDeleteDialogProps) {
  if (!row) {
    return null;
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/30 px-4">
      <div className="w-full max-w-md rounded-lg bg-white p-5 shadow-xl">
        <h2 className="mt-2 text-xl font-bold text-slate-950">
          Hapus {row.kepala_keluarga}
        </h2>
        <p className="mt-2 text-sm leading-6 text-slate-500">
          Apakah Anda yakin ingin menghapus data anggota BMT ini?
        </p>
